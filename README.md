...
**A playable English build exists.** `tools/patch_all.py` writes the dialogue, all
three entry-1098 tables, the UI and the font in a single glyph-allocation pass;
`tools/verify_patch.py` proves the packed archive changed only what it claimed to.
Dialogue, items, spells, skills, plot items, menus, the status screen, the battle
UI, character names and the shop are all in English.

Still outstanding: the title-screen artwork (the image descriptor table has never
been found), the new-game text crawl, and a few strings the engine writes from
code rather than from any table.
...
- **Entries 190, 208, 367, 831, 881, 893, 933.** Four or five "messages" each that
  decode as junk under the script parser — either not script entries, or a format
  nobody has read. The new-game text crawl is probably one of them.
- **無裝備 and the equipment dash filler**, written from code rather than any table
  found so far.
...
tools/
  ...
  names.py        item / spell / skill names in entry 1098
  patch_all.py    build the English archive - one allocation pass, all text
  insert_english.py  dialogue line-setter, used by patch_all
  verify_patch.py    diff a packed archive against a clean one
...
data/
  charmap.json    1,688 verified index → character mappings
  digraphs.json   generated by patch_all: which Latin pair lives in which slot
